...
async function employeeLogin() {
    ...
    let httpResponse = await fetch("http://localhost:8080/ExpenseReimbursementSystem/api/LoginPage?username=" + login.userName, settings);
    let user = await httpResponse.json();
    console.log(user)

    if (user.employeeId != 0) {
        window.location.href = "http://localhost:8080/ExpenseReimbursementSystem/EmployeeHome.html";
    }
    else {
...
async function managerLogin() {
    ...
    let httpResponse = await fetch("http://localhost:8080/ExpenseReimbursementSystem/api/ManagerLoginPage?username=" + login.userName, settings);
    let user = await httpResponse.json();
